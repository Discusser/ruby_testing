...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        inputs = ['&', '3']
        allow(game_input).to receive(:gets).and_return(*inputs)
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        min = game.instance_variable_get(:@minimum)
        max = game.instance_variable_get(:@maximum)
        expect(game.verify_input(min, max, 2)).to eq(nil)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      previous_guess_count = game.instance_variable_get(:@guess_count)
      game.display_turn_order(binary_search)
      expect(previous_guess_count + 1).to eq(game.instance_variable_get(:@guess_count))
    end

...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 18 and the random number is 11' do
      subject(:game) { MagicSeven.new(11) }
      it 'returns 7' do
        previous_step = 18
  ...
  describe '#play' do
    context 'when the random number is 13' do
      subject(:game) { MagicSeven.new(13) }
    ...
    context 'when the random number is 17' do
      subject(:game) { MagicSeven.new(17) }
      # remove the 'x' before running this test
      it 'will return 7' do
        expect(game.play).to eq(7)
      end
